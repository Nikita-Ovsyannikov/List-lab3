...
public:
	...
	//Zadacha: vstavit' v uporyadochennyy spisok element, ne izmenyaya poryadka, 
	// za odin prokhod. po vremeni - O(n), po pamyati - O(1)
	// variant 1
	void sort() {
		if (!first || !first->next) {
			return; 
		...
		while (curr) {
			...
			while (tmp) {
				if (tmp->value < minNode->value) {
					minNode = tmp;
				}
				tmp = tmp->next;
			}
			std::swap(curr->value, minNode->value);
			curr = curr->next;
		}
	}


	void insert_in_sorted_list(const T& val)
	{
		...
		auto it = begin();
		while (it.getNode()->next && it.getNode()->next->value < val) {
			++it;
		}

		Node<T>* newNode = new Node<T>(val, it.getNode()->next);
